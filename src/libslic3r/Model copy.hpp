...
private:
    ...
    ModelObject(const ModelObject &rhs) : ObjectBase(-1), config(-1), layer_height_profile(-1), m_model(rhs.m_model), mmu_colors(rhs.mmu_colors) { 
    	assert(this->id().invalid()); 
        assert(this->config.id().invalid()); 
        assert(this->layer_height_profile.id().invalid());
	...
	template<class Archive> void serialize(Archive &ar) {
        ...
        ar(name, input_file, instances, volumes, config_wrapper, layer_config_ranges, layer_heigth_profile_wrapper, 
            ...
            m_bounding_box_approx, m_bounding_box_approx_valid, mmu_colors,
...
private:
    ...
    std::string base7056list = "123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!@#$%^&*()-_+=[{]};:,<>";
