...
static constexpr const char* COLORS_ATTR = "colors";
static constexpr const char* RESOURCES_TAG = "m:color";
...
static constexpr const char* THREEMF_COLOR_ATTR = "color";
...
static constexpr const char* MMU_SEGMENTATION_OLD_ATTR = "slic3rpe:mmu_segmentation";
static constexpr const char* MMU_SEGMENTATION_ATTR = "paint_colors";
...
namespace Slic3r {
    ...
    class _3MF_Importer : public _3MF_Base
    {
        ...
        std::string m_name;
        std::string mmu_colors;
        int m_extruder_cnt = 0;
        int mmu_color_cnt = 0;
    ...
    bool _3MF_Importer::_load_model_from_file(const std::string& filename, Model& model, DynamicPrintConfig& config, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        mz_zip_archive_file_stat start_part_stat { 0 };
            ...
            if (obj_metadata != m_objects_metadata.end()) {
                ...
                for (const Metadata& metadata : obj_metadata->second.metadata) {
                    if (metadata.key == "name")
                        model_object->name = metadata.value+ (mmu_colors!=""?"=="+mmu_colors:"");
                    else
...
//        model.adjust_min_z();
    ...
    {
        if(get_attribute_value_string(attributes, num_attributes, THREEMF_COLOR_ATTR)!=""){
            std::string colstr = get_attribute_value_string(attributes, num_attributes, THREEMF_COLOR_ATTR);
            colstr.erase(6,2);
            mmu_colors += std::to_string(mmu_color_cnt++)+"--"+colstr+"$$";
        }
    ...
    bool _3MF_Importer::_handle_end_object()
    {
        if (m_curr_object.object != nullptr) {
            ...
            if (m_curr_object.geometry.empty()) {
                ...
                if(mmu_colors!="")
                    m_curr_object.object->name += "=="+mmu_colors;
                //m_curr_object.object->config.set("has_mixing_colors",true);
    ...
    bool _3MF_Importer::_handle_start_triangle(const char** attributes, unsigned int num_attributes)
    {
        ...
        if(get_attribute_value_string(attributes, num_attributes, MMU_SEGMENTATION_OLD_ATTR)!="") 
            m_curr_object.geometry.mmu_segmentation.push_back("O"+get_attribute_value_string(attributes, num_attributes, MMU_SEGMENTATION_OLD_ATTR));
        else if(get_attribute_value_string(attributes, num_attributes, MMU_SEGMENTATION_ATTR)!="")
        ...
        else if(get_attribute_value_string(attributes, num_attributes,P1_ATTR)!="")
            m_curr_object.geometry.mmu_segmentation.push_back("Z"+std::to_string(2+get_attribute_value_int(attributes, num_attributes, P1_ATTR)));//else if(get_attribute_value_string(attributes, num_attributes, MMU_SEGMENTATION_ATTR)!="") m_curr_object.geometry.mmu_segmentation.push_back("S"+get_attribute_value_string(attributes, num_attributes, MMU_SEGMENTATION_ATTR));
        else m_curr_object.geometry.mmu_segmentation.push_back(get_attribute_value_string(attributes, num_attributes, MMU_SEGMENTATION_ATTR));
    ...
    bool _3MF_Importer::_handle_end_metadata()
    {
        ...
        } else if (m_curr_metadata_name == "ColorHash") {
            mmu_colors = m_curr_characters;
    ...
    bool _3MF_Importer::_generate_volumes(ModelObject& object, const Geometry& geometry, const ObjectMetadata::VolumeMetadataList& volumes, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
            ...
            object.name += "==" + mmu_colors; 
            BOOST_LOG_TRIVIAL(info) << "loaded "<< object.name;
    ...
    bool _3MF_Exporter::_add_model_file_to_archive(const std::string& filename, mz_zip_archive& archive, const Model& model, IdToObjectDataMap& objects_data)
    {
...

        {
            ...
            if (model.is_mm_painted()){// && wxGetApp().plater().has_mixing_extruders()){
                std::string hash;
                std::string last_color;
                for(auto &obj: model.objects)
                    if(obj->mmu_colors.size()>0)
                        for(std::pair<int,std::string>& color: obj->mmu_colors){
                            if(last_color == "" || color.second != last_color)
                                hash += std::to_string(color.first)+"--"+(color.second)+"$$";
                            last_color = color.second;
                        }
                if(hash!="") 
                    stream << " <" << METADATA_TAG << " name=\"ColorHash\">" << hash << "</" << METADATA_TAG << ">\n";
            }
    ...
    bool _3MF_Exporter::_add_model_config_file_to_archive(mz_zip_archive& archive, const Model& model, const IdToObjectDataMap &objects_data)
    {
        ...
        for (const IdToObjectDataMap::value_type& obj_metadata : objects_data) {
            ...
            if (!obj->name.empty()){                    
                std::vector<std::string> cleaned = split(obj->name,"==");
                add_metadata(stream, 2, MetadataType::object, "name", cleaned[0]);
            }
