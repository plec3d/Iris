...
std::string FacetsAnnotation::get_triangle_as_string(int triangle_idx) const
{
    ...
    if (triangle_it != m_data.first.end() && triangle_it->first == triangle_idx) {
        ...
        while (offset < end) {
            ...
            std::string digit = base7056list[std::floor(next_code/84)]+base7056list[next_code%84];
...
void FacetsAnnotation::set_triangle_from_string(int triangle_id, const std::string& str, int extruder_cnt, std::vector<int> *color_map)
{
    std::string instr = str.size()==0?"2":str.c_str();
    int c_type = (instr.compare(0,1,"Z") == 0)? 2: (instr.compare(0,1,"S") == 0)? 1: 0;
    BOOST_LOG_TRIVIAL(info) << c_type << " " << (m_data.second.size());
    if(instr.size() > 1 && c_type > 0)
        instr.erase(0,1);
    assert(! instr.empty());
    ...
    for (auto it = instr.crbegin(); it != instr.crend(); ++it) {
        ...
        if((it == instr.crbegin() || (c_type == 1 && it == instr.crbegin()+1)) && dec > 2 && color_map != nullptr){
            //BOOST_LOG_TRIVIAL(info) << "deca " << dec;
            int deca = dec - 3;
            std::vector<int>::iterator it = std::find_if (color_map->begin(), color_map->end(), [deca](int i) {
                return (deca == i);
            });
            if(it != color_map->end())
                dec = std::distance(color_map->begin(),it) + extruder_cnt;
