...
std::string GCodeWriter::toolchange(unsigned int extruder_id, const std::vector<std::string>& mixing_colors, const std::string& target_color)
{
    ...
    std::ostringstream gcode;
    // replace all mixing extruder toolchanges to actual Mix gcode (M165)
    if(mixing_colors.size()>0){
        ColorRGB color;
        decode_color(target_color, color);
        gcode << this->toolchange_prefix() << extruder_id << "\n";
        gcode << (color.generate_mix_gcode(mixing_colors)).c_str() << /*" T" + std::to_string(extruder_id) +*/ " ; color: " << color.r()*(1/0.878431)*255 << " " << color.g()*(1/0.878431)*255 << " " << color.b()*(1/0.878431)*255 << "\n";
...
std::string GCodeWriter::travel_to_xyz(const Vec3d &point, const std::string_view comment)
{
    ...
    if (std::abs(m_to_lift) > EPSILON) {
        ...
        if (this->is_current_position_clear() && delta(2) > 0 && delta_no_z.norm() != 0.0f) {
            ...
            //BBS: LazyLift
                ...
                //check whether we can make a travel like
                //   _____
    ...
    //take plate offset into consider
    this->set_current_position_clear(true);
