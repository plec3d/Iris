...
const std::array<ColorRGBA, static_cast<size_t>(GCodeExtrusionRole::Count)> GCodeViewer::Extrusion_Role_Colors{ {
    ...
    { 1.00f, 0.50f, 0.50f, 1.0f },   // GCodeExtrusionRole::OverhangInfill
...
void GCodeViewer::reset_shell()
{
    ...
    m_shells.print_id = -1;
    m_shells_bounding_box = BoundingBoxf3();
}
...
void GCodeViewer::render_legend(float& legend_height)
{
    ...
    if (!view_type_changed) {
        ...
        case EViewType::Tool:                 {
            std::vector<int> virt_exts = wxGetApp().virtual_extruders();
            // shows only extruders actually used
            for (unsigned char extruder_id : m_extruder_ids) {
                if (used_filaments_m[extruder_id] > 0.0 && used_filaments_g[extruder_id] > 0.0)
                    append_item(EItemType::Rect, m_tool_colors[extruder_id], _u8L(virt_exts.at(extruder_id)>-1?"Mixer":"Extruder") + " " + std::to_string(virt_exts.at(extruder_id)>-1?virt_exts.at(extruder_id):extruder_id + 1) + (virt_exts.at(extruder_id)> -1?" mix":""),
