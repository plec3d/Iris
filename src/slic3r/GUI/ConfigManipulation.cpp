...
void ConfigManipulation::toggle_field(const std::string& opt_key, const bool toggle, int opt_index/* = -1*/)
{
    if (local_config) {
        if (local_config->option(opt_key) == nullptr)
            return;
    	if((opt_key.compare("support")|| opt_key.compare("overhang"))&&!opt_key.compare("overhang_auto_setting")) local_config->set_key_value("ignore_overhang_auto_setting",new ConfigOptionBool(true));
    	else if(opt_key.compare("overhang_auto_setting")) local_config->set_key_value("ignore_overhang_auto_setting",new ConfigOptionBool(false));
    }
...
void ConfigManipulation::update_print_fff_config(DynamicPrintConfig* config, const bool is_global_config)
{
    ...
    bool ask_autofill = wxGetApp().preset_bundle->prints.get_selected_preset().config.option<ConfigOptionBool>("auto_fill_mixing_colors")->value; 
    if (wxGetApp().has_mixing_extruders() && ask_autofill){
        wxString msg_text = _(L("The loaded model has colors defined. Shall I load those colors on your mixing extruder?"));
        MessageDialog dialog (m_msg_dlg_parent, msg_text, _(L("Mixing extruder coloring")),
                                wxICON_WARNING | wxYES | wxNO);
        auto answer = dialog.ShowModal();
        if (answer == wxID_YES){
            DynamicPrintConfig new_conf = *config;
            new_conf.set_key_value("has_mixing_colors", new ConfigOptionBool(true));
            apply(config, &new_conf);
        }                      
